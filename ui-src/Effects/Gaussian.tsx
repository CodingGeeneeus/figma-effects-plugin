...
const makeGaussKernel = (sigma:number) => {
  const GAUSSKERN:number = 6.0;

  let dim:number = Math.max(3.0, GAUSSKERN * sigma);
  var sqrtSigmaPi2 = Math.sqrt(Math.PI*2.0)*sigma;
  var s2 = 2.0 * sigma * sigma;
  var sum = 0.0;
  
  var kernel:Float32Array = new Float32Array(dim - (dim & 1)); // Make it odd number
...
*/
function gauss_internal(pixels:ImageData, kernel:any, ch:number, gray:boolean){
...
*/
export function gauss(canvas:HTMLCanvasElement, sigma:number){
